...
public class HDLProxy extends HttpServlet {
    ...
    {
        ArrayList<HashMap<String, String>> arXiv = new ArrayList<>();
        {
            HashMap<String, String> landingpage = new HashMap<>();
            {
                landingpage.put("display", "landingpage");
                landingpage.put("url", "https://arxiv.org/abs");
            }

            HashMap<String, String> metadata = new HashMap<>();
            {
                metadata.put("display", "metadata");
                metadata.put("url", "http://export.arxiv.org/oai2?verb=GetRecord&metadataPrefix=oai_dc&identifier=");
            }

            HashMap<String, String> resource = new HashMap<>();
            {
                resource.put("display", "resource");
                resource.put("url", "https://arxiv.org/pdf/");
            }

            arXiv.add(landingpage);
            arXiv.add(metadata);
            arXiv.add(resource);
        }
        metaresolverMappingConfigs.put("arXiv", arXiv);
    }
    ...
    private boolean urnDeMatchFound = false;
    private boolean urnFiMatchFound = false;
    private boolean arkMatchFound = false;
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    private void handleNonMatchingRequest(HttpServletRequest req, HttpServletResponse resp, HDLServletRequest hdl) throws IOException, ServletException {
        String accept = req.getHeader("Accept");
        String contentType = req.getContentType();
        if (accept != null && accept.toUpperCase().contains(Common.HDL_MIME_TYPE.toUpperCase()) &&
                contentType != null && contentType.toUpperCase().contains(Common.HDL_MIME_TYPE.toUpperCase())) {
    ...
    private void forwardToNativeServlet(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        // Implement the logic to forward the request to the native servlet
